...
function rVRTE(){
  ...
  var barHtml=meses.map(function(m,i){
    ...
    return '<div style="display:flex;flex-direction:column;align-items:center;gap:2px;flex:1">'
      ...
      +'<div style="flex:1;background:var(--gn2);height:'+Math.max(pe,2)+'px;border-radius:2px 2px 0 0" title="Entradas: '+m.ent+'"></div>'
      +'<div style="flex:1;background:var(--rd2);height:'+Math.max(ps,2)+'px;border-radius:2px 2px 0 0" title="Saídas: '+m.sai+'"></div>'
      +'</div>'
      +'<span style="font-size:9px;color:var(--t3)">'+m.label+'</span>'
      +'</div>';
  }).join('');
  document.getElementById('v-grafico').innerHTML=barHtml;

  // Consumo por operação
  var opHtml=Object.keys(opConsumo).length?Object.entries(opConsumo).sort(function(a,b){return b[1]-a[1];}).map(function(kv){
    var pct=sai>0?Math.round(kv[1]/sai*100):0;
    return '<div style="margin-bottom:8px">'
      +'<div style="display:flex;justify-content:space-between;font-size:11px;margin-bottom:2px"><span>'+kv[0]+'</span><span style="font-family:var(--mo);color:var(--rd)">'+kv[1].toLocaleString('pt-BR')+' ('+pct+'%)</span></div>'
      +'<div style="background:var(--s2);border-radius:3px;height:6px"><div style="background:var(--rd);height:100%;width:'+pct+'%;border-radius:3px"></div></div>'
      +'</div>';
  }).join(''):'<div style="color:var(--t3);font-size:12px;text-align:center;padding:12px">Nenhum dado ainda</div>';
  document.getElementById('v-op').innerHTML=opHtml;

  // Tabela mensal resumo
  var tblMes=meses.map(function(m){
    return '<tr><td>'+m.label+'</td>'
      +'<td><span class="badge bgg">+'+m.ent.toLocaleString('pt-BR')+'</span></td>'
      +'<td><span class="badge bgr">-'+m.sai.toLocaleString('pt-BR')+'</span></td>'
      +'<td style="font-weight:500">'+(m.ent-m.sai).toLocaleString('pt-BR')+'</td>'
      +'</tr>';
  }).join('');
  document.getElementById('v-tmeses').innerHTML=tblMes;
...
function regVRTE(){
  ...
  var v=DB.vrte;v.saldo+=q;v.hist.push({d,t:'E',q,s:v.saldo,r:r||'Entrada manual'});DB.vrte=v;
  document.getElementById('vq').value='';document.getElementById('vo').value='';show('va');rVRTE();
}

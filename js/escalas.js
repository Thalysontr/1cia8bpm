// ═══ ESCALAS ═══
function rEscs(){
  var escs=APP.escs.slice().sort(function(a,b){return new Date(b.data)-new Date(a.data);});
  ...
  tb.innerHTML=escs.length?escs.map(function(e){
    return '<tr>'
      ...
      +'<td style="white-space:nowrap;display:flex;gap:4px;align-items:center">'
        +'<button class="btn bsm brd" onclick="gerarPDFById('+e.id+')">PDF</button>'
        +'<button class="btn bsm bg2" onclick="gerarDocxById('+e.id+')">DOCX</button>'
...
function gerarPDFById(id){var e=APP.escs.find(function(x){return x.id===id;});if(e)gerarPDF(e);}
function gerarDocxById(id){var e=APP.escs.find(function(x){return x.id===id;});if(e)gerarDocx(e);}

function confirmarExcluirEscala(id){
  var esc=APP.escs.find(function(x){return x.id===id;});
  if(!esc)return;
  var msg='Excluir a escala "'+esc.op+'" do dia '+fd(esc.data)+'?\n\nEsta ação irá estornar '+esc.vrte+' VRTE de volta ao saldo.';
...
function excluirEscala(id){
  ...
  esc.turnos.forEach(function(t){
    (t.mils||[]).forEach(function(m){
      var f=APP.mils.find(function(x){return x.rg===m.rg;});
      if(f&&f.hist){
  ...
  // Deletar escala
  DB.deleteEsc(id,function(){
    reloadEscs(function(){ rEscs(); rPainel(); });
  });
}
